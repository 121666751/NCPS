...
/**
 * @ClassName: AT0190000903
 * @Description: 取现操作通知
 * @author Q
 * @date 2018年3月27日下午9:57:52
 */
public class AT0190000903 {
    
    private static QrBook pubBook = new QrBook();

    public static int chk() throws Exception {
	SysPub.appLog("INFO", "取现操作通知开始");
	...
	try {
	    ...
	    SysPub.appLog("ERROR", "AT01900000903chk失败");
	    e.printStackTrace();
	    throw e;
	}
	EPOper.put(szTpID, "INIT._FUNC_RETURN", 0, "0");
	SysPub.appLog("INFO", "取现操作通知成功");
	return 0;
    }
...
//	EPOper.put(szTpID, "OBJ_QRUP_ALL[0].respMsg", "初始化错误");
    ...
    public static int transDeal()
    {
		...
		SysPub.appLog("INFO", "执行 0190000903-transDeal 方法开始");

		String svrReq = "OBJ_EBANK_SVR_QR0190_REQ";
		String cltReq = "OBJ_QRUP_ALL";
		String svrRes = "OBJ_EBANK_SVR_QR0190_RES";
	...
	SysPub.appLog("INFO", "开始AT0190000903业务处理");
	DtaInfo dtaInfo = DtaInfo.getInstance();
	String tpID = dtaInfo.getTpId();
	...
	SysPub.appLog("INFO", "开始AT0190000903登记簿数据");
	int iResult = 0;
	DtaInfo dtaInfo = DtaInfo.getInstance();
	String tpID = dtaInfo.getTpId();
	...
	try {
	    ...
	    // 调度银联CUP0303服务
	    SysPub.appLog("INFO", "调用0190000903服务开始");
	    DtaTool.call("QRUP_CLI", "AT0190");
	    
	    
	    
	} catch (Exception e) {
	    EPOper.put(tpID, "INIT._FUNC_RETURN", 0, "-1");
	    SysPub.appLog("ERROR", "调用银联服务0190000903失败：%s", e.getMessage());
	    return -1;
	    //throw e;
	}
	EPOper.put(tpID, "INIT._FUNC_RETURN", 0, "0");
