...
//		    QrBusiPub.putPubRet("01", "原订单信息没有找到",svrRes);
	...
	stepDeal( svrReq,  svrRes,  tpID,  cltReq, qrBook,  cltRes,  platDate,  iseq_no);
	
	// 组织数据登记到book表
	ret = insQrBook( qrBook,  platDate,  iseq_no,svrRes+"[0].cd[0].respCode", svrRes+"[0].cd[0].respMsg");
	if (ret != 1) {
...
//	ret = uptOrderInfo();
    ...
    private static int stepDeal(String svrReq, String svrRes, String tpID, String cltReq, QrBook qrBook, String cltRes,
			String platDate, int iseq_no) throws Exception {
    	// 查询原交易的订单信息
    	int flag = 0;
    	if (qryOrderInfo(svrReq,   svrRes,  qrBook) != 0) {
    		//本应此处return，为保证录入输入报文数据到t_qrp_book表中，在sendUP中判断return
			flag =  -1;
		} 
    	
    	// 组织报文发送到银联
    	if(sendUP( svrReq,  cltReq, svrRes,  cltRes,qrBook,flag) != 0) {
    		return -1;
    	}
    	
    	if ( recvUp( svrRes, cltRes, tpID,  qrBook,  platDate,  iseq_no,svrReq) != 0) {
    		return -1;
    	}
    	return 0 ;
	}
     ...
     * @throws Exception
     */
    private static int recvUp(String svrRes,String cltRes,String tpID, QrBook qrBook, String platDate, int iseq_no, String svrReq) throws Exception  {
    	// 非对象类型赋值
...
//	    优惠信息
	    ...
	    if(offstAmt != null && !offstAmt.trim().isEmpty()) {
	   		EPOper.put(tpID, svrRes+"[0].cd[0].couponInfo[0].offstAmt", AmountUtils.changeF2Y(offstAmt));
	   	}else {
	   		EPOper.put(tpID, svrRes+"[0].cd[0].couponInfo[0].offstAmt", "");
	   	}
//	    EPOper.put(tpID, svrRes+"[0].cd[0].couponInfo[0].offstAmt", (String) EPOper.get(tpID, "COUPONINFO[0].offstAmt"));
	...
	try {
		...
		if (amt / 100 > 0.01) {
		    qrBook.setCoupon_info_offst_amt(Double.valueOf(AmountUtils.changeF2Y(offstAmtStr)));
     ...
     */
    public static int sendUP(String svrReq, String cltReq,String svrRes, String cltRes, QrBook qrBook, int flag) throws Exception {
...
//	    EPOper.copy(tpID, tpID, "T_QRP_BOOK[0].CURRENCY_CODE", "OBJ_QRUP_ALL[0].currencyCode");
		...
		String respCode = "01";
		String respMsg = "调用银联服务0180000903失败";
	    EPOper.put(tpID, svrRes+"[0].cd[0].respCode",respCode);
		EPOper.put(tpID, svrRes+"[0].cd[0].respMsg",respMsg);
		qrBook.setResp_code(respCode);
		qrBook.setResp_msg(respMsg);
