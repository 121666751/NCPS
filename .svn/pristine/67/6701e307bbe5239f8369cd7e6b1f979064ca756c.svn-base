...
public class PubDel {
	...
	public static int deal_trans() throws Exception {
		...
		try {
			...
			String returnMsgObj = "";
			ArrayList<NetbankConf> trade;// 交易属性
			...
			SysPub.appLog("INFO", "trade size+" + trade.size() + "svcName:" + svcName);
			...
			for (NetbankConf netbankConf : trade) {
				...
				if ("RES".equals(type) && type_CHECK != null && type_CHECK.startsWith("$")) {
			...
			EPOper.put(tpID, "ISO_8583[0].pub[0].PlatSeqNo", strSeq);// 流水号
																		// 传递到host_cli文件传输时采用文件名(操作员号+渠道号+流水号)
			...
			/* 交易报文检查 */
			if (chk_trans(tpID, svrReq, svrRes, trade) < 0) {
			...
			/* 调用核心对应得交易 */
			if (callHost(tpID, cltReq, cltRes, svrReq, svrRes, trade,startTime) < 0) {
				return -1;
			}
			...
			for (NetbankConf netbankConf : trade) {
				...
				if ("RES".equals(type) && "1".equals(type_PURPOSE)) {// 找到 ,
					...
					String elemType = netbankConf.getTYPE_DATATYPE();
					// 用途为“发报文”的配置内容
					...
					String type_cnt = netbankConf.getTYPE_CONTENTSCONT();
					int cnt = 0;
					if (!StringTool.isNullOrEmpty(type_cnt)) {
						if (isInteger(type_cnt)) {
							cnt = Integer.valueOf(type_cnt);
						} else {
							String szCnt = (String) EPOper.get(tpID, type_cnt);// 平台流水号
							if (szCnt != null && !"".equals(szCnt.trim()))
								cnt = Integer.valueOf(szCnt);
						}
					}
					String type_CHECK = netbankConf.getTYPE_CHECK();
					...
					if (file != null && !"".equals(file.trim())) {// 为FILE时
						if (cnt == 0) {
							EPOper.put(tpID, svrRes + "[0].cd[0].rowsNum", 0);// 组循环次数
							continue;
						}
						...
						SysPub.appLog("INFO", "to:[%s] value:[%s][%s]", dealObj(to), DealExpr(goal, type_EXPR),
								elemType);
						Object tmp = DealExpr(goal, type_EXPR);
						if (tmp.equals("null")) {
							if (!StringTool.isNullOrEmpty(elemType)) {
								if ("Double".equals(elemType))
									EPOper.put(tpID, dealObj(to), "0.00");
							} else
								EPOper.put(tpID, dealObj(to), "");
						} else {
	...
	private static int oganizeFomatFile(NetbankConf netbankConf, String tpID, String svrRes, String svrReq)
			throws Exception {
		...
		for (int j = 0; j < rowsNumInt; j++) {
			...
			for (int i1 = 0; i1 < typeContentsArr.length; i1++) {
				...
				// EPOper.put(tpID, svrRes + type_ROOT + "[" + j + "]." +
				// typeContentsArr[i1], fileList.get(j)[i1]);
			}
	...
	private static int oganizeFomat(NetbankConf netbankConf, String tpID, String svrRes, String svrReq, long startTime)
		...
		while ((readLine = bufferedReader.readLine()) != null) {
			...
			if (type_FILEBEGIN <= i) {// 设置数据起始笔数
				readLine = readLine.replaceAll(separator, " " + separator);
		...
		for (int j = 0; j < fileList.size(); j++) {
			for (int i1 = 0; i1 < typeContentsArr.length; i1++) {
				SysPub.appLog("INFO", "第[%s]行，内容为：[%s]", j, svrRes + type_ROOT + "[" + j + "]." + typeContentsArr[i1],
						fileList.get(j)[i1]);
				SysPub.appLog("INFO", "value:[%s]", fileList.get(j)[i1]);
				EPOper.put(tpID, svrRes + type_ROOT + "[" + j + "]." + typeContentsArr[i1], fileList.get(j)[i1].trim());
			}
		}
	 ...
	 * @throws Exception
	 */
	private static int chk_trans(String tpID, String svrReq, String svrRes, ArrayList<NetbankConf> trades)
			throws Exception {
		SysPub.appLog("INFO", "chk_trans	begin");
		for (NetbankConf netbankConf : trades) {
			String type_FROM = netbankConf.getTYPE_FROM();
			String type = netbankConf.getTYPE();
	 ...
	 * @throws Exception
	 */
	public static int callHost(String tpID, String cltReq, String cltRes, String svrReq, String svrRes,
			ArrayList<NetbankConf> trades, long startTime) throws Exception {
		...
		String nextto = trades.get(0).getNEXTTO();
		try {
			SysPub.appLog("ERROR", "hostCode:[%s]", hostCode);
		...
		if (!check.equals(retCode1)) {
			...
			EPOper.put(tpID, svrRes + "[0].hostErrorMessage", retMsg1 == null ? "交易失败" : retMsg1);
