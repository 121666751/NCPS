...
public class GW0006 {
	...
	public static int deal_trans() throws Exception {
		try {
			...
			//客户化步骤（实现理财金额合计，电子账户余额查询
			tranOthDeal(tpID,svrRes,svrReq,cltRes);
	...
	public static int tranOthDeal(String tpID,String svrRes,String svrReq, String cltRes) throws Exception{
...
//      在途止付不体现在理财资产里面——2016年7月25日16:52:50 @贾汶川
		...
		//在途金额
		String szOnLoad= (String) EPOper.get(tpID, cltRes+ "[0].D122A");
		//持仓金额
		String szOwned= (String) EPOper.get(tpID, cltRes+ "[0].D122E");
		//理财总资产
		BigDecimal dbOnLoad = new BigDecimal(szOnLoad);
		BigDecimal dbOwned = new BigDecimal(szOwned);
		dbOwned =dbOwned.add(dbOnLoad);
		szOwned = dbOwned.toString();
		
		EPOper.put(tpID,  svrRes+"[0].cd[0].lcAmt",szOwned); //总金额
		
		/* 上核心查询 电子账户余额*/
		SysPub.appLog("INFO", "STEP1 去核心[%s]检查付款账户", "740006");
		

		EPOper.put(tpID,   "ISO_8583[0].iso_8583_010","EBNK");//前台终端号		
		EPOper.put(tpID,   "ISO_8583[0].iso_8583_016","7400");//交易代码


		//客户号
		String custNo = (String) EPOper.get(tpID,    svrReq+"[0].cd[0].custNo");
		custNo = custNo.substring(1);
		EPOper.put(tpID,  "ISO_8583" + "[0].iso_8583_028",custNo);
		//EPOper.copy(tpID, tpID,  svrReq+"[0].cd[0].custNo", "ISO_8583[0].iso_8583_028");
        //虚拟卡号
		EPOper.copy(tpID, tpID,  svrReq+"[0].cd[0].virtualAcctNo", "ISO_8583[0].iso_8583_032");
		
		EPOper.put(tpID,  "ISO_8583" + "[0].iso_8583_053","6"); //核心交易码
		EPOper.copy(tpID, tpID,  "ISO_8583", "OBJ_ALA_abstarct_REQ[0].req");
		try {
			DtaTool.call("HOST_CLI", "S740006");
		} catch (Exception e) {
		...
		String actBal = (String) EPOper.get(tpID,   "ISO_8583" + "[0].iso_8583_043");//电子账号余额
		String szAmt=AmountUtils.changeF2Y(actBal);
		
		int flag = szAmt.indexOf(".");
		if( flag == -1){
			szAmt = szAmt+".00";
		}

		EPOper.put(tpID,  svrRes+"[0].cd[0].virtualAmt",szAmt); //总金额
